 ...
 async function initHoodMap(opts){
  ...
  function render(type='all'){
   ...
   groups.forEach(rows=>{
    ...
    rows.forEach(row=>{const item=document.createElement('div');item.style.marginBottom='12px';const title=document.createElement('strong');title.textContent=row.title||'נכס';const detail=document.createElement('p');detail.textContent=[row.location,row.priceLabel||row.price].filter(Boolean).join(' · ');const a=document.createElement('a');a.href='index.html?prop='+encodeURIComponent(row.id);a.textContent='לפרטי הנכס ←';item.append(title,detail,a);popup.append(item);});
